...
# Live feed. 1.1.16 is the version the 2026-08-20 outage nearly lost.
Check 'published version resolves'        (Get-FeedState '1.1.16')   'present'
...
$src  = $real.Definition -replace 'https://community\.chocolatey\.org/api/v2/Packages\(Id=''antigravity-cli'',Version=''\$Version''\)', 'https://community.chocolatey.org/api/v2/$Version'
Set-Item function:Get-FeedStateStub -Value ([ScriptBlock]::Create($src))
Check 'server error is unknown, not absent' (Get-FeedStateStub 'Packages(Id=%27antigravity-cli%27)') 'unknown'
